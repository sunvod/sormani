...
class Page_pool(list):
  ...
  def clean_images(self, color, threshold, final_threshold, last_threshold, use_ai, thresh_threshold, min_threshold):
    if len(self):
      start_time = time.time()
      dir_name = self.filedir.split('/')[-1]
      if self.date is not None:
      ...
      for page in self:
        ...
        page.valid = None
        page.use_ai = use_ai
        page.thresh_threshold = thresh_threshold
        page.min_threshold = min_threshold
      with Pool(processes=N_PROCESSES) as mp_pool:
  ...
  def cut_at_white_part(self, threshold, color, limit, var_limit, x_ofset):
    for page in self:
      page.threshold = threshold
      page.color = color
      page.limit = limit
  ...
  def center_block(self, threshold, color, use_ai, only_x, borders):
    for page in self:
      page.threshold = threshold
      page.color = color
      page.use_ai = use_ai
