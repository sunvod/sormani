...
hookspec = pluggy.HookspecMarker('sormani')
...
def initialize(plugin_manager: pluggy.PluginManager):
    ...
    plugins and possibly block other blocks, a plugin that wishes to block sormani's
    built-in optimize plugin could do:

    .. code-block::

        plugin_manager.set_blocked('sormani.builtin_plugins.optimize')
    ...
    Raises:
        sormani.exceptions.ExitCodeException: If options are not acceptable
...
def add_options(parser: ArgumentParser) -> None:
    ...
    to ``sormani.ocr``, or when invoked on the command line.
...
def validate(pdfinfo: PdfInfo, options: Namespace) -> None:
    ...
    *options* contains the "work order" to process a particular file. *pdfinfo*
    ...
    Raises:
        sormani.exceptions.ExitCodeException: If options or pdfinfo are not acceptable
            and the application should terminate gracefully with an informative
            message and error code.
    Note:
        This hook will be called from the main process, and may modify global state
