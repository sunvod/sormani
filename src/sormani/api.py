...
"""Functions for using sormani as an API."""
...
class Verbosity(IntEnum):
    ...
    debug = 1  #: Output sormani debug messages
    debug_all = 2  #: More detailed debugging from sormani and dependent modules
...
def configure_logging(
    ...
    *,
    ...
    Before calling :func:`sormani.ocr()`, you can use this function to
    configure logging if you want sormani's output to look like the sormani
    ...
    script for sormani and you want it to be very similar to sormani, use this
    function; if you are using sormani as part of an application that manages
    its own logging, you probably do not want this function.

    If this function is not called, sormani will not configure logging, and it
    is up to the caller of ``sormani.ocr()`` to set up logging as it wishes using
    ...
    Regardless of whether this function is called, sormani will perform all of
    its logging under the ``"sormani"`` logging namespace. In addition,
    sormani imports pdfminer, which logs under ``"pdfminer"``. A library user
    ...
    Returns:
        The toplevel logger for sormani (or the root logger, if we are managing it).
    """

    prefix = '' if manage_root_logger else 'sormani'
...
def ocr(  # pylint: disable=unused-argument
    ...
    **kwargs,
    ...
    Raises:
        sormani.MissingDependencyError: If a required dependency program is missing or
            was not found on PATH.
        sormani.UnsupportedImageFormatError: If the input file type was an image that
            could not be read, or some other file type that is not a PDF.
        sormani.DpiError: If the input file is an image, but the resolution of the
            image is not credible (allowing it to proceed would cause poor OCR).
        sormani.OutputFileAccessError: If an attempt to write to the intended output
            file failed.
        sormani.PriorOcrFoundError: If the input PDF seems to have OCR or digital
            text already, and settings did not tell us to proceed.
        sormani.InputFileError: Any other problem with the input file.
        sormani.SubprocessOutputError: Any error related to executing a subprocess.
        sormani.EncryptedPdfError: If the input PDF is encrypted (password protected).
            OCRmyPDF does not remove passwords.
        sormani.TesseractConfigError: If Tesseract reported its configuration was not
            valid.

    Returns:
        :class:`sormani.ExitCode`
    """
    ...
    with _api_lock:
        # We can't allow multiple sormani.ocr() threads to run in parallel, because
        ...
        if 'verbose' in kwargs:
            warn("sormani.ocr(verbose=) is ignored. Use sormani.configure_logging().")
